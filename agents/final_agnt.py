...
# ---------------------------------------------------------------------------
# Scenario labels.
# KNOWN: the 4 scenarios we have tuned strategies for. detect_scenario fires
# fast on alert keywords and downstream code already handles them well.
# UNKNOWN: 6 hidden scenarios revealed at test time. When alerts are present
# but no known-scenario keyword fires, we ask the LLM to classify into one of
# these so we at least carry a meaningful label forward.
KNOWN_SCENARIOS = ("baseline", "supply_crisis", "tourist", "renovation")
UNKNOWN_SCENARIOS = (
    "black_swan",
    "feast_or_famine",
    "health_scare",
...
# Cheap keyword fast-path for the 6 unknowns. Catches obvious cases without
# burning an LLM call. If none match AND alerts exist, fall through to LLM.
_UNKNOWN_KW = {
    "black_swan":      ("black swan", "catastrophe", "disaster", "extreme", "unprecedented"),
    "feast_or_famine": ("feast", "famine", "boom and bust", "volatile", "swing"),
    "health_scare":    ("health", "scare", "inspection", "food safety", "contamination", "outbreak"),
    "inflation":       ("inflation", "price increase", "cost rise", "rising cost", "price hike"),
    "premium_pivot":   ("premium", "upscale", "luxury", "fine dining", "high-end", "pivot"),
    "silent_drift":    ("drift", "subtle", "gradual", "slow change", "creeping"),
}


# ---------------------------------------------------------------------------
# Lazy LLM client for the unknown-scenario classifier. Built on first use so
# import stays fast and a missing API key never crashes the agent.
...
def _llm():
    global _LLM_CLIENT, _LLM_FAILED
    if _LLM_FAILED:
        return None
...
def _llm_pick_unknown_scenario(alerts_text: str, day: int) -> str | None:
    """Ask the LLM to label an unknown scenario from the 6 candidates.

    Returns one of UNKNOWN_SCENARIOS, or None if the LLM is unavailable or
    returned garbage. Only called when (a) alerts are present, (b) no known
    keyword matched, and (c) no unknown keyword matched — so this is a rare,
    focused call.
    """
    client = _llm()
    if client is None:
        return None
    options = ", ".join(UNKNOWN_SCENARIOS)
    sys_msg = (
        "You classify restaurant simulator alerts into one of these unseen "
        f"scenario labels: {options}. The four known scenarios "
        "(baseline, supply_crisis, tourist_season, renovation) have ALREADY "
        "been ruled out by keyword match. Pick the single best-fitting label "
        "from the list above. Respond with ONLY the label, no punctuation."
    )
    user_msg = f"Day {day}/30. Alert text:\n{alerts_text[:1500]}"
    try:
        ...
        label = label.strip(" \t\n.,;:'\"")
        if label in UNKNOWN_SCENARIOS:
            return label
        for u in UNKNOWN_SCENARIOS:
            if u.replace("_", " ") == label or u == label.replace(" ", "_"):
                return u
    except Exception as e:
        print(f"  [scenario LLM] day {day}: {type(e).__name__}: {str(e)[:80]}")
    return None
...
def detect_scenario(observation: dict, state: dict, day: int) -> str:
    ...
    # --- KNOWN 4: tuned-strategy fast-path. If any of these match, return
    # immediately so the existing per-scenario heuristics drive today's actions.
    ...
    if any(w in text for w in ("tourist", "surge", "festival", "event", "influx", "boom")):
        return "tourist"
    # NOTE: inflation/health_scare are KEPT here as legacy known-4 fallbacks
    # only because downstream code may already handle them lightly. They're
    # also among the unknown_6 list, so the same label still resolves.
    ...
    # --- UNKNOWN 6: no known scenario fired. If alerts are present (so SOMETHING
    # is happening), try cheap keyword match on the 6 hidden scenarios, then
    # ask the LLM if needed. Cache the result in state["sc_llm"] so we only
    # query the LLM once per game per change in alert text.
    if alerts and text.strip():
        for label, keywords in _UNKNOWN_KW.items():
            if any(k in text for k in keywords):
                state["sc"] = label
                return label

        # LLM fallback. Cache key includes the alert text so a new alert
        # triggers a re-query but identical alerts do not.
        cache_key = hash(text)
        cached = state.get("sc_llm")
        if isinstance(cached, list) and len(cached) == 2 and cached[0] == cache_key:
            return cached[1] or (current or "baseline")
        picked = _llm_pick_unknown_scenario(text, day)
        state["sc_llm"] = [cache_key, picked]
        if picked:
            state["sc"] = picked
            return picked
